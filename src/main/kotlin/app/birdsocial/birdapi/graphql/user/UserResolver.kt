...
class UserResolver(val sessionFactory: SessionFactory) {
	...
	fun users(@Argument userArgs: UserArgs): List<UserGQL> {
		if (true) { // BirdApiApplication.bucket.tryConsume(1)
			...
			val usersN4J = usersN4JUnfiltered.filter { user ->
			...
			val users: List<UserGQL> = usersN4J.map { user ->
				user.toUserGQL()
			}
//			println("USER")
			val endTime = System.nanoTime()
			println("T: ${(endTime-startTime)/1_000_000.0} D: ${(filterTime-startTime)/1_000_000.0} F: ${(endTime-filterTime)/1_000_000.0}")
